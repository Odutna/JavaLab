...
/**
 ...
 Poiché la chiamata al costruttore di PrintWriter può generare un’eccezione di tipo FileNotFoundException,
 essa deve essere inclusa in un blocco try. Le eventuali eccezioni sono gestite nel blocco catch.
 Anche se il costruttore dovesse generare un’eccezione di questo tipo, non è detto che essa sia dovuta al
 fatto che il file non è stato trovato (d’altronde, è previsto che se il file non esiste già ne venga creato
 uno nuovo). In tal caso, l’eccezione significherebbe che non è stato possibile creare il file, per esempio
 perché il nome specificato è già utilizzato come nome di una directory.

 Una volta che il file è stato aperto (cioè una volta che è stato collegato a uno stream di output) è possibile
 scrivervi dei dati. Il metodo println della classe PrintWriter consente di scrivere dati in un file di testo
 esattamente nello stesso modo in cui il metodo System.out.println consente di scrivere sullo schermo.
 La classe PrintWriter ha anche un metodo print che si comporta esattamente come il metodo System.out.print,
 ad eccezione del fatto che in questo caso l’output è scritto su file.
...
public class EditaFraseFileDiTesto {
    ...
    public void connettiFileInput() {
        ...
        try {
            ...
            System.out.println("Errore: il file di input " + nomeFileInput + " non e' stato trovato.");
            System.exit(0);
        }
    }
