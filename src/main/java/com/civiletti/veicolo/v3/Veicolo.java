...
public class Veicolo {
    ...
    // Velocità massima con una data marcia
    private static final int[] VELOCITA_MASSIMA_PER_MARCIA = {0, 10, 29, 45, 70, 180};

    private String verificaCambioMarcia() {
        if (marcia < 1 || marcia >= VELOCITA_MASSIMA_PER_MARCIA.length) return "";

        int velocitaMassimaCorrente = VELOCITA_MASSIMA_PER_MARCIA[marcia];
        int velocitaMinimaSuccessiva = (marcia > 1) ? VELOCITA_MASSIMA_PER_MARCIA[marcia - 1] : 0;
        String avviso = "";
        if (velocita > velocitaMassimaCorrente) {
            avviso += "Attenzione!!! passare alla [" + (marcia+1) +"]";

//            stampaMessaggioCambioMarcia("Attenzione!!! passare alla [" + (marcia+1) +"]");
        } else if (velocita < velocitaMinimaSuccessiva) {
            avviso += "Attenzione!!! passare alla [" + (marcia-1) +"]";
//            stampaMessaggioCambioMarcia("Attenzione!!! passare alla [" + (marcia-1) +"]");
        }
        return avviso;
    ...
    // Metodo per stampare un messaggio lampeggiante
    private String stampaMessaggioCambioMarcia(String messaggio) {
//        System.out.print("\033[31;5m" + messaggio + "\033[0m\n");
//        return "\033[7m\033[5;31m" + messaggio + "\033[0m";
        return "\033[7m\033[38;5;37m" + messaggio + "\033[0m";
    }
    ...
    public void accelera() {
        if (acceso && marcia > 0 && velocita < 180) {
            ...
            // Controlla se è necessario cambiare marcia
            verificaCambioMarcia();
...
//                System.out.println("Scalata marcia. Marcia: " + marcia);
    ...
    public void retromarcia() {
        if (acceso && velocita == 0) {
            marcia = -1;
            System.out.println("[R] Inserita retromarcia");
        } else {
            System.out.println("[R] Indietro - Simuala la retromarcia che non può essere inserita");
        }
    }
...
//        System.out.println("=== \033[32m  QUADRO INDICATORI  \033[0m ===");
        ...
        System.out.println(" MArcia: " + marcia + " " + stampaMessaggioCambioMarcia(verificaCambioMarcia()) ) ;
        System.out.println(" Chilometri percorsi: \033[7m " + String.format("%.2f", chilometri) + " km \033[0m");
